  ...
  const handleMouseMove = useCallback(
    (movement: { endPosition: { x: number; y: number } }) => {
      ...
      if (defined(picked) && picked.id && typeof picked.id.id === 'string') {
        ...
        // Check entity name for infrastructure (entity.id is auto-generated by GeoJsonDataSource)
        const entity = picked.id;
        const entityName: string = entity.name ?? '';
        if (entityName.startsWith('infra-route-')) {
          const displayName = entityName.split('::')[1] || 'Infrastructure';
